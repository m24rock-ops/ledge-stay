...
function ensureLoginUiElements() {
  ...
  if (!otpSection) {
    ...
    otpSection.innerHTML = `
      ...
      <input type="text" id="login-name" placeholder="Your name (e.g. Manoj, Priya)">
      <div style="text-align:right;margin-top:6px;">
        <a href="#" id="resend-otp-link" style="font-size:13px;color:#1a6fb0;text-decoration:none;">Resend OTP</a>
      </div>
    `;
...
function bindLoginUiEvents() {
  ...
  document.getElementById('resend-otp-link')?.addEventListener('click', async (e) => {
    ...
    const timer = setInterval(() => {
      link.textContent = `Resend OTP (${seconds}s)`;
      seconds--;
      if (seconds < 0) {
        clearInterval(timer);
