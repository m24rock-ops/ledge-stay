...
function renderListingCard(listing) {
  ...
  const ownerPhone = listing.contact || listing.owner?.whatsapp || listing.owner?.phone || '';
  const waMsg = encodeURIComponent(`Hi, I'm interested in your property: ${card.title}. Please share details.`);
  const waUrl = ownerPhone ? `https://wa.me/91${ownerPhone.replace(/\D/g,'')}?text=${waMsg}` : '';
      ...
      <div class="card-body">
        ...
        <div class="card-footer">
          ...
          <div class="card-actions" onclick="event.stopPropagation()">
            ${waUrl ? `<a class="btn-whatsapp" href="${waUrl}" target="_blank" rel="noopener" onclick="event.stopPropagation()">WhatsApp</a>` : ''}
...
async function loadFeaturedListings() {
  ...
  try {
    ...
    featuredGrid.innerHTML = listings.map((listing) => {
      const ownerPhone = listing.contact || listing.owner?.whatsapp || listing.owner?.phone || '';
      const waMsg = encodeURIComponent(`Hi, I'm interested in your property: ${listing.title}. Please share details.`);
      const waUrl = ownerPhone ? `https://wa.me/91${ownerPhone.replace(/\D/g,'')}?text=${waMsg}` : '';
      return `
      <article class="featured-card featured-card--hero" data-id="${listing._id}" onclick="showDetail('${listing._id}')">
        <div class="featured-image-wrap">
        ...
        <div class="featured-card-body">
          ...
          <div class="featured-card-footer">
            <div class="featured-price-block">
              <span class="featured-price-label">Starting from</span>
              <div class="featured-price">₹${Number(listing.price).toLocaleString()}/month</div>
            </div>
            <div class="featured-card-actions" onclick="event.stopPropagation()">
              ${waUrl ? `<a class="featured-wa-button" href="${waUrl}" target="_blank" rel="noopener">WhatsApp</a>` : ''}
              <button class="featured-view-button" onclick="event.stopPropagation();showDetail('${listing._id}')">See details</button>
            </div>
          </div>
        </div>
      </article>`;
...
function renderPopularLocations() {
  ...
  grid.innerHTML = HOME_LOCATIONS.map((location) => {
    const areaName = location.name.replace(' 📍', '').trim();
    return `
    <article class="location-card" onclick="applyPopularLocation('${escapeHtml(areaName)}')">
      <img src="${location.image}" alt="PG rooms in ${escapeHtml(areaName)} Bengaluru" loading="lazy" decoding="async">
      <div class="location-card-overlay"></div>
      <div class="location-card-copy">
        <p class="location-card-city">Bengaluru</p>
